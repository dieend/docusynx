...
export async function discoverDocuments(input: {
    ...
    for (const value of objectValuesDeep(input.allContent)) {
        ...
        if (versions) {
            for (const versionValue of versions) {
                ...
                if (!version) continue
                const versionName =
                    stringProperty(version, 'versionName') ??
                ...
                for (const docValue of docs) {
                    const candidate = await candidateFromMetadata(
                        docValue,
                        input.siteDir,
                        prefix,
                        nextOrder++
                    )
                    if (candidate) documents.set(candidate.id, candidate)
                }
                applySidebars(
                    version.sidebars,
                    documents,
    ...
    for (const route of [...input.routePaths].sort()) {
        ...
        if (
            matchesAny(normalizedRoute, exclusions) ||
            matchesAny(route, exclusions)
        )
            continue
        if (routeSet.has(normalizedRoute)) continue
    ...
    for (const document of documents.values()) {
        if (
            matchesAny(
                document.route,
                input.options.renderedRoutePatterns ?? []
            )
        )
...
async function candidateFromMetadata(
    ...
    const route =
        stringProperty(metadata, 'permalink') ??
        stringProperty(metadata, 'route')
    const rawSource =
        stringProperty(metadata, 'source') ??
        stringProperty(metadata, 'sourcePath')
    if (!route || !rawSource) return undefined
    ...
    const rawId =
        stringProperty(frontMatter, 'docusynx_id') ??
        stringProperty(metadata, 'id') ??
        routeId
    const sourceOverride =
        stringProperty(frontMatter, 'source_path') ??
    ...
    return {
        ...
        sourcePath:
            sourceOverride ?? normalizeSourcePath(siteDir, absolute, rawId),
...
function walkSidebar(
    ...
    items.forEach((itemValue, index) => {
        ...
        const id =
            linkedId && documents.has(linkedId)
                ? linkedId
                : `${prefix}category:${slug([...ancestry, label].join('/'))}`
        if (!documents.has(id)) {
            const route =
                stringProperty(link ?? {}, 'slug') ??
                `/.docusynx/category/${slug([...ancestry, label].join('/'))}/`
            const generatedIndexRoute =
                stringProperty(link ?? {}, 'type') === 'generated-index'
                    ? normalizeRoute(route)
                    : undefined
            documents.set(id, {
                id,
...
function indexDocumentId(
    ...
    for (const itemValue of itemsValue) {
        const item = asRecord(itemValue)
        if (!item || stringProperty(item, 'type') !== 'doc') continue
        const rawId = stringProperty(item, 'id') ?? ''
        const basename = rawId.split('/').at(-1)
        if (basename !== 'index' && basename !== 'overview') continue
...
function reconcileGeneratedIndexRoutes(
    documents: Map<string, CandidateDocument>,
    routePaths: string[]
): void {
    const normalizedRoutes = [...new Set(routePaths.map(normalizeRoute))].sort()
    for (const document of documents.values()) {
        if (!document.generatedIndexRoute) continue
        const suffix = document.generatedIndexRoute
        const matches = normalizedRoutes.filter(
            (route) => route === suffix || route.endsWith(suffix)
        )
        if (matches.length > 1) {
            throw new Error(
                `generated-index category ${document.id} matches multiple rendered routes: ${matches.join(', ')}`
            )
        }
...
export async function readRenderedRoute(
    outDir: string,
    route: string
): Promise<string> {
...
async function renderedRouteExists(
    outDir: string,
    route: string
): Promise<boolean> {
    for (const candidate of renderedRouteCandidates(outDir, route)) {
...
function renderedRouteCandidates(outDir: string, route: string): string[] {
...
function objectValuesDeep(value: unknown): UnknownRecord[] {
...
function resolveSourcePath(siteDir: string, source: string): string {
...
function compareCandidate(
    left: CandidateDocument,
    right: CandidateDocument
): number {
