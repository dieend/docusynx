...
export interface LinkInline {
    type: 'link'
    target: { kind: 'url' | 'document'; value: string }
    children: Inline[]
}
...
export type Block =
    ...
    | { type: 'admonition'; kind: string; title?: string; blocks: Block[] }
    | { type: 'image'; assetId: string; alt?: string; title?: string }
    | { type: 'thematicBreak' }
    | { type: 'extension'; name: string; data: unknown }
...
export interface ComponentHandler {
    ...
    ): Block | Block[] | Promise<Block | Block[]>
}
...
export function defineComponentHandler(
